...
#ifndef CCNX_RATE_HOP_DELAY_TRACER_H
#define CCNX_RATE_HOP_DELAY_TRACER_H
 ...
 */
class L3HopDelayTracer : public L3Tracer {
protected:
  struct HopsStats {
    inline void
    Reset()
    ...
    std::string m_inInterests;
    std::string m_outInterests;
    std::string m_inData;
    std::string m_outData;
    std::string m_inNack;
    std::string m_outNack;
  };
public:
  /**
   ...
   */
  L3HopDelayTracer(shared_ptr<std::ostream> os, Ptr<Node> node);
   ...
   */
  L3HopDelayTracer(shared_ptr<std::ostream> os, const std::string& node);

  /**
   * @brief Destructor
   */
  virtual ~L3HopDelayTracer();
  ...
  virtual void
  Print(std::ostream& os, std::pair<nfd::FaceId, std::tuple<HopsStats, HopsStats>> stats) const;
  void
...
private:
  ...
  mutable std::map<nfd::FaceId, std::tuple<HopsStats, HopsStats>> m_stats;
...
#endif // CCNX_RATE_HOP_DELAY_TRACER_H
