...
#ifndef NDN_DATA_BOLT_H
#define NDN_DATA_BOLT_H
...
public:
   ...
   */
  DataBolt(KeyChain& keyChain): a_face(nullptr, keyChain), m_keyChain(keyChain), 
															 m_scheduler(a_face.getIoService()){
		std::cerr << "DataBolt::Binding..." << std::endl;
		// a_face.setInterestFilter("/Trace", 
		// 												std::bind(&DataBolt::onInterest, this,  _1, _2),
		// 												RegisterPrefixSuccessCallback(),
		// 												std::bind(&DataBolt::onRegisterFailed, this, _1, _2));
		a_face.setInterestFilter("/Trace",
                             bind(&DataBolt::onInterest, this, _1, _2),
                             RegisterPrefixSuccessCallback(), // RegisterPrefixSuccessCallback is optional
                             bind(&DataBolt::onRegisterFailed, this, _1, _2));
  }
  ~DataBolt(){}
...
protected:
  ...
  std::string
	...
	// std::cerr<< "Looped Interest for Nonce: " << interest.getNonce()<< std::endl;
		...
		// std::cout << "DataBolt::onInterest" << std::endl;
		struct Quest r;
		...
		if (Reqs[interest.getNonce()].p1=="M"){ // Could be from User (no face_id) or from another daemon
			...
			const ::ndn::Name nx = c.toUri() ;
			::ndn::Name v = nx.toUri();
      std::cout << "DataBolt::OnInterest::" << interest.getName().toUri() << std::endl;
			...
			std::cout << "DataBolt::OnInterest::newName:" << n.toUri() << ", Nonce:" << interest.getNonce()<< std::endl;
			I.setName(n);
      ...
      m_scheduler.schedule(ndn::time::seconds(0), [I, this] {
                a_face.expressInterest(I,
                                       bind(&DataBolt::onData, this,  _1, _2),
                                       bind(&DataBolt::onNack, this, _1, _2),
                                       bind(&DataBolt::onTimeout, this, _1));
      });
			return;
		}
	}
	...
	{
		std::cout<< "DtaBolt::onData: name=" << data.getName().toUri() << std::endl;
		double wait =0;
    ...
    std::cout<< "DataBolt::dataprocessing: " <<   data_x <<", " << "name=" << dataName.toUri() << std::endl;
    clean(interest.getNonce());
